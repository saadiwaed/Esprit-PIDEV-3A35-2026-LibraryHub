...
/**
 * Client HTTP dédié au bibliothécaire virtuel (indépendant du module Forum).
 */
final class VirtualLibrarianAiClient
{
    ...
    public function requestJson(string $feature, string $systemPrompt, string $userPrompt): ?array
    {
        ...
        try {
            ...
            if ($statusCode >= 400) {
                $this->logger->warning('Virtual librarian AI provider returned an HTTP error.', [
                    'feature' => $feature,
            ...
            if (!is_array($payload)) {
                $this->logger->warning('Virtual librarian AI provider returned an invalid JSON payload.', [
                    'feature' => $feature,
                    'provider' => $this->provider,
                ]);
            ...
            if ($rawContent === null || $rawContent === '') {
                $this->logger->warning('Virtual librarian AI provider did not return a usable message content.', [
                    'feature' => $feature,
                    'provider' => $this->provider,
                ]);
            ...
            if (!is_array($decoded)) {
                $this->logger->warning('Virtual librarian AI provider message content is not valid JSON.', [
                    'feature' => $feature,
                    'provider' => $this->provider,
                ]);
            ...
            $this->logger->info('Virtual librarian AI provider response received.', [
                'feature' => $feature,
                'provider' => $this->provider,
                'model' => $this->model,
            ...
            $this->logger->warning('Virtual librarian AI provider call failed.', [
                'feature' => $feature,
                'provider' => $this->provider,
    ...
    private function logMissingConfigurationOnce(): void
    {
        ...
        $this->logger->warning('Virtual librarian assistant is disabled due to missing configuration.', [
            'provider' => $this->provider,
